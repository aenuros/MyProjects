...
function isFull(array) {
  for(i=0;i<array.length;i++) {
    if(undefined == typeof(array[i]) || null == array[i] || "" == array[i]) {
      return false;
    }
  }
  return true;
...
function smoothieRoll() {
  ...
  if (smoothieDice === 2) {
    console.log(">>>You got a SMOOTHIE! If your health is low, a SMOOTHIE will heal you completely!");
      ...
      for(i=0;i<yourstatus.arrayMaxValues;i++) {
        if(yourstatus.arrayOfStuff[i] === "") {
          yourstatus.arrayOfStuff[i] = "smoothie";
          break;
        }
      }


    //yourstatus.arrayOfStuff.shift("smoothie");
    inventoryDisplay();
...
while (battleState == 0) {
  ...
  if (movement==="f") {
    ...
    if (isFull(yourstatus.arrayOfStuff) ==false) {
      smoothieRoll();
    }
    ...
    if (dice!=5) {
      ...
      if(isFull(yourstatus.arrayOfStuff)) {
      ...
      let turn = 0;
        ...
        if (turn === 0) {
          if (yourstatus.arrayOfStuff.length > 0) {
            ...
            if (attack==="i") {
              inventoryDisplay();
              console.log(yourstatus.arrayOfStuff);
            }
